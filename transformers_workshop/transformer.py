...
def correction_fn(expansion_ratio: float, hidden_size: int) -> int:
...
def swiglu_ln_ffn(hidden_size: int, expansion_ratio: float, dropout: float = 0.1):
    return nn.Sequential(
        ...
        Linear(
            hidden_size, correction_fn(expansion_ratio, hidden_size) * 2
        ),
        SwiGLU(),
        nn.Dropout(dropout),
        Linear(correction_fn(expansion_ratio, hidden_size), hidden_size),
    )
